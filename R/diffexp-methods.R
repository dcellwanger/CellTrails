...
.diffExprState_def <- function(x, state1, state2, feature_name, alternative="two.sided", lod=NULL) {
...
#' @keywords internal
#' @author Daniel C. Ellwanger
.diffExprTrail_def <- function(x, feature_name, trail_names, n=250, score, dynamicFit_k=5, df=10) {
  ...
  #Fetch trail info
  path_1 <- x@trails[[trail_names[1]]]$path
  ptime_1 <- x@trails[[trail_names[1]]]$ptime
  samples_1 <- x@trails[[trail_names[1]]]$samples
  path_2 <- x@trails[[trail_names[2]]]$path
  ptime_2 <- x@trails[[trail_names[2]]]$ptime
  samples_2 <- x@trails[[trail_names[2]]]$samples

  #Fit dynamics
  if(max(ptime_1) < max(ptime_2)) {
    reference <- .fitDynamic_def(x=ptime_1, y=x[feature_name, samples_1],
                                 z=states(x)[samples_1], k=dynamicFit_k) #, x.out = ptime_1
    query <- .fitDynamic_def(x=ptime_2, y=x[feature_name, samples_2],
                             z=states(x)[samples_2], k=dynamicFit_k) #, x.out = ptime_1
    reference_name <- trail_names[1]
    query_name <- trail_names[2]
    ptime <- ptime_2
    query$mod <- query$gam
  } else {
    reference <- .fitDynamic_def(x=ptime_2, y=x[feature_name, samples_2],
                                 z=states(x)[samples_2], k=dynamicFit_k) #, x.out = ptime_2
    query <- .fitDynamic_def(x=ptime_1, y=x[feature_name, samples_1],
                             z=states(x)[samples_1], k=dynamicFit_k) #, x.out = ptime_2
  ...
  if(var(reference$y) == 0 | var(query$y) == 0){ #any gene is constant
    ...
    if(var(reference$y) == 0) {
      if(max(ptime_1) < max(ptime_2)) {
        query <- .fitDynamic_def(x=ptime_2, y=x[feature_name, samples_2],
                                 z=states(x)[samples_2], k=dynamicFit_k, n.out=length(n))
      } else {
        query <- .fitDynamic_def(x=ptime_1, y=x[feature_name, samples_1],
                                 z=states(x)[samples_1], k=dynamicFit_k, n.out=length(n))
      }
  ...
  # First and last elements of the query are anchored at the boundaries of the reference (= global alignment)
  ...
  df <- min(length(unique(query$y[alignment$index1])) - 1, length(unique(reference$y[alignment$index2])) - 1)
  mod <- smooth.spline(query$x[alignment$index1], query$y[alignment$index1], df=df) #, tol = 1e-7
  query_aligned <- predict(mod, n)
  query_aligned$y[query_aligned$y < 0] <- 0
  query_aligned$mod <- mod

  mod <- smooth.spline(query$x[alignment$index1], reference$y[alignment$index2], df=df) #, tol = 1e-7
...
#' @param alternative Alternative hypothesis for P-value simulation (less, greater, two.sided)
...
.find_marker_def <- function(x, k=NULL, B=1000, alternative="greater") {
  ...
  f.snr <- function(x, sel) {
    ...
    # }
    #snr <- f.snr_statistic(x[sel], x[!sel]) #(mean(x[sel]) - mean(x[!sel])) / (sd(x[sel]) + sd(x[!sel]))
