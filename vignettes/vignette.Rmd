...
subtitle: Daniel Christian Ellwanger and Stefan Heller
author: 
...
# EXAMPLE DATASET
<!-- ---------------------------------- -->
The *CellTrails* package comes with an example dataset (of class `Biobase::ExpressionSet`), which can be loaded with the function `data`.
...
To block the cell cycle effects, *CellTrails* expects a design matrix modeling the cell cycle stage as the explanatory factor for each cell. As the cell-cycle stage of each cell is not known in this data set, we need to predict cell cycle phases. In this example, we use the classifier `cyclon` from the *scran* package [@scran][^8]. To be able to run the algrithm properly, gene symbols were translated to Ensembl identifiers using Bioconductors' annotation database interface package *AnnotationDbi* [@annotationdbi][^9] and the mouse annotation data package *org.Mm.eg.db* [@orgmmegdb][^10]. Please note that these packages are not part of *CellTrails* and may needed to be installed first (see footnotes for details).
...
If the user prefers to use an alternative approach for dimensionality reduction, the latent space can be set to a `CellTrailsSet` object with `latentSpace<-`. The latent space has to be a numerical matrix; rows represent samples and columns the components of the latent space. *CellTrails* uses by default spectral embedding, but the framework also operates well with any other spectral dimensionality reduction method, such as PCA (available in *CellTrails* via function `pca`) and diffusion maps (available via the *destiny* package [@destiny][^11]):
...
We found that the freely available graph visualization software _yEd_[^12] has great capabilities to visualize and analyze a trajectory graph. An optimal layout is planar, i.e. it exhibits no crossing edges or overlapping nodes.
...
## PARALLELIZATION
In the case one wants to compare a large number of features (e.g. from an RNA-Seq experiment), the computation can be significantly sped up by parallel computing. In this example, we use the package _doSNOW_[^13], but any other package may also be used for this purpose.
...
[^12]: http://www.yworks.com/products/yed
