...
For your convenience, the numeric expression matrix (here called `th2`) and the list of marker genes were already organized in a `r Biocpkg("SingleCellExperiment")` container. Here, the expression matrix consists of 7,063 selected genes (116 of which are marker genes) which have been detected in more than 2 of all 81 cells.
```{r, eval=TRUE}
...
The function `plotManifold` returns a list. It contains a `ggplot` object [@ggplot2] from the `r CRANpkg("ggplot2")` package, which can be adapted by the user's needs and preferences, and the tSNE parameters and result. To lower computational time, a previous tSNE result can be stored and re-used for future manifold visualizations.
```{r, eval=TRUE}
# Store tSNE result
latentSpaceSNE(exBundle) <- tsne_result

# Show feature expression (e.g., genes TECTA and MYO7A)
plotManifold(exBundle, color_by="featureName", name="TECTA", only_plot=TRUE)
plotManifold(exBundle, color_by="featureName", name="MYO7A", only_plot=TRUE)
```

*Please note* that the parameter `only_plot` is used to indicate that only the plot should be shown and the tSNE results and parameters shouldn't be returned.
...
# Show manifold
plotManifold(exBundle, color_by="phenoName", name="state", only_plot=TRUE)
```
...
pth <- system.file("img", "yEd_1.gif", package="CellTrails") #"img/yEd_1.gif"
...
```{r, eval=TRUE, echo=FALSE}
pth <- system.file("img", "yEd_2-3.gif", package="CellTrails") #"img/yEd_2-3.gif"
knitr::include_graphics(path=pth) #, dpi=300 system.file("doc", "yEd_2.gif", package="CellTrails")) #
```

Let's run the algorithm to compute the layout:

```{r, eval=TRUE, echo=FALSE}
pth <- system.file("img", "yEd_4.gif", package="CellTrails") #"img/yEd_4.gif"
knitr::include_graphics(path=pth)
```
...
```{r, eval=TRUE, echo=FALSE}
pth <- system.file("img", "yEd_5-6.gif", package="CellTrails") #"img/yEd_6-7.gif"
knitr::include_graphics(path=pth)
...
```{r, eval=TRUE}
# Get trail names
plotManifold(exBundle, color_by="phenoName", name="TrS", only_plot=TRUE)
```
...
```{r, eval=TRUE, echo=FALSE}
pth <- system.file("img", "yEd_7.gif", package="CellTrails") #"img/yEd_10.gif"
knitr::include_graphics(path=pth)
```
...
```{r, eval=TRUE, echo=FALSE}
pth <- system.file("img", "yEd_8.gif", package="CellTrails") #"img/yEd_11-12.gif"
knitr::include_graphics(path=pth)
```
...
```{r, eval=TRUE, echo=FALSE}
pth <- system.file("img", "yEd_9.gif", package="CellTrails") #"img/yEd_9.gif"
knitr::include_graphics(path=pth)
```
...
plotManifold(bundle, color_by="phenoName", name="fm143", only_plot=TRUE)
plotManifold(bundle, color_by="featureName", name="OTOA", only_plot=TRUE)
plotManifold(bundle, color_by="featureName", name="CALB2", only_plot=TRUE)
...
plotManifold(p1utricle, color_by="phenoName", name="gate", only_plot=TRUE)
plotManifold(p1utricle, color_by="featureName", name="Otoa", only_plot=TRUE)
plotManifold(p1utricle, color_by="featureName", name="Fscn2", only_plot=TRUE)
