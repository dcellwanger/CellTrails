...
### Spike-in Controls
There is no need to remove spike-in controls from your `r Biocpkg("SingleCellExperiment")`. *CellTrails* automatically ignores spike-in controls for its analysis, if they were properly annotated in the object using the function `isSpike` (see _Example datasets_).

## Example Data Sets
In this vignette we use simulated data (using log-transformed Negative Binomial distributed expression counts) and real expression data to illustrate the functionality of the *CellTrails* package. 

The first dataset, `exSim`, is composed of expression values of 15,000 features measured in 100 samples under the identical condition; 80 spike-in transcripts were added.
```{r, eval=TRUE}
...
<!-- For illustration purposes, we assumed that we were able to observe for some samples their "maturity" indirectly by a discrete cellular characteristic (e.g., the FM1-45 dye uptake by mature sensory hair cells, or the cell stage during embryogenesis). Please note, that this metainformation is _not_ used by _CellTrails_ for trajectory reconstruction, but it helps to better understand the workflow described in this vignette. 
```{r, eval=TRUE}
# Create SingleCellExperiment object
exSmall <- exDat()
show(exSmall)
...
```{r, eval=TRUE}
showTrajInfo(exSmall)
```

The entries _logcounts_ and _Feature data_ correspond to the expression matrix and feature information as provided and annotated by the user, respectively. _Pheno data_ contains metainformation for each sample stored by the user and by *CellTrails*. The entries _Trajectory data_, _Trajectories_ and _Trail data_ denote *CellTrails* specific information and will be described in detail in the respective section of this vignette.

<!-- ---------------------------------- -->
# Selection of Trajectory Features
<!-- ---------------------------------- -->
An expression matrix from an high-throughput experiment contains hundreds of features that bear no or little information about a samples's progress through the trajectory of interest. These non-relevant features not only increase the computational runtime of the trajectory reconstruction, but they also impair the accuracy of downstream analysis results. *CellTrails* assumes that key features show high expression variability during a samples's progression along the biological process under study. Therefore, *CellTrails* enables to unbiasedly filter the informative features. 

*Please note*, that the following functions only filter features used for dimensionality reduction, state detection and trajectory reconstruction; it does not remove features from the assay stored in the `r Biocpkg("SingleCellExperiment")` object. Thus, all features are still available for *CellTrails* donwstream analyses capabilities, such as cluster analysis, inference of gene expression dynamics, as well as, for functions available from complementary analysis packages.
...
The function `trajFeatureNames` allows to designate any set of features as trajectory features. The only requirement is that the names in the set match the stored feature names in the `r Biocpkg("SingleCellExperiment")` object (check with function `featureNames`). In example, we could use the package `r Biocpkg("scran")` [@scran] to filter features:

```{r, eval=TRUE}
# Filter using scran
var_fit <- scran::trendVar(x=exSim)
...
# Diffusion maps
dmaps_result <- destiny::DiffusionMap(t(logcounts(exAlt)), n_eigs = 101)
d <- findSpectrum(dmaps_result@eigenvalues, frac=100)
latentSpace(exAlt) <- dmaps_result@eigenvectors[, d]
```
...
## Visualization
*CellTrails* allows us to visualize an approximation of the learned lower-dimensional manifold in two dimensions. *CellTrails'* plot function `plotManifold` uses t-distributed stochastic neighbor embedding [@vdmaaten2008] to illustrate the arrangement of the samples in the latent space in a two-dimensional plot. Points denote individual samples, the colorization indicates either a metadata label or expression of a single feature. Empty points denote a missing label or missing expression value (non-detects). The optional parameters `seed` and `perplexity` are passed to the Barnes-Hut implementation of t-distributed stochastic neighbor embedding (tSNE). Available phenotype lables can be listed with the function `phenoNames`, available features with `featureNames`, respectively.
...
tsne_result <- plotManifold(exBundle, color_by="phenoName", name="fm143")
```

The function `plotManifold` returns a list. It contains a `ggplot` object [@ggplot2] from the `r CRANpkg("ggplot2")` package, which can be adapted by the user's needs and preferences, and the tSNE result. To lower computational time, a previous tSNE result can be stored and re-used for future manifold visualizations.
```{r, eval=TRUE}
# Store tSNE result
latentSpaceSNE(exBundle) <- tsne_result

# Show feature expression (e.g., genes TECTA and MYO7A)
tsne_result <- plotManifold(exBundle, color_by="featureName", name="TECTA")
tsne_result <- plotManifold(exBundle, color_by="featureName", name="MYO7A")
```
...
State assignments are stored as sample metainformation and can be either recieved via `colData` or `states`. Since `CellTrails` operates on a `r Biocpkg("SingleCellExperiment")` object, its results can easily used by other packages. For example, visualizing a principal component analysis with `r Biocpkg("scater")`:
```{r, eval=TRUE}
...
## Using Alternative Methods
Technically, the function `states<-` allows to set any clustering result to a `r Biocpkg("SingleCellExperiment")` object. Any numeric, character or factor vector having the cluster assignments for each sample is accepted.
...
# Show manifold
tsne_result <- plotManifold(exBundle, color_by="phenoName", name="state")
```
...
The inferred state trajectory graph can be visualized using `plotStateTrajectory`. If the graph is a forest, the parameter `component` can be used to define which tree should be shown. Let's have a look at the FM1-43 uptake and the _CALB2_ expression in component 1:
...
For the sake of simplicity and performance, it makes sense to conduct subsequent steps for each component individually. In this case, we select the tree formed by graph component 1 with 896 cells (accessible via *trajSampleNames*) for our example data analysis.
...
The function `trajSampleNames` returns the names of the samples which were selected for trajectory reconstruction. If further details are of interest for this set of samples, the `r Biocpkg("SingleCellExperiment")` object can be simply subset:
```{r, eval=TRUE}
# Subset to get sample info
...
As expected, state _S3_ is not contained in this subset.
...
*Please note*, that edge crossings (i.e., two or more edges overlap) are not useful and if they occur we suggest to re-run the layouter with different parameters before saving the layout. Using either your mouse or the _View_ > _Zoom In_ option allows to have a closer look. If we want to have the trajectory progressing from bottom left to bottom right (based on a specific gene expression or phenotype label), we need to transform the graph. This can be done via _Tools_ > _Geometric transformations_. Here, we select _Mirror on Y axis_ and _Mirror on X axis_.
...
Finally, the file can be saved via _File_ > _Save_ and reimported to R. 
...
# Plot layout
plot(tl[,1:2], axes=F, xlab="", ylab="", pch=20, cex=.25)
```
...
plot(trajLayout(exBundle), axes=F, xlab="", ylab="", pch=20, cex=.25)
```
...
## Plot Maps
After generating the layout, a two-dimensional visualization of the trajectory can be drawn. Here, the line represents the trajectory and individual points represent the samples. This plot type either colorizes individual samples by a metadata label (as available via `phenoNames`) or it shows the topography of a given feature's expression landscape (as available via `featureNames`). When metadata are being visualized, the grey line represents the trajectory and the individual points represent the samples Samples that do not have a specific metadata label or value are not shown. Let's visualize how the cellular FM1-43 uptake distributes along the trajectory:
```{r, eval=TRUE}
...
We may want to have a look into the standard error of the predicted expression surface using the parameter.
```{r, eval=TRUE}
...
```{r, eval=TRUE}
# Get trail names
tsne_result <- plotManifold(exBundle, color_by="phenoName", name="TrS")
```

Also, the data is accessible via `colData` of the `r Biocpkg("SingleCellExperiment")` object and can therefore, be analyzed using alternative packages (e.g., `r Biocpkg("scater")`).
...
The boxplot statistics indicate that there is a strong outlier in state _S9_ (which is termed state _i_ in the original *CellTrails* article). Let's extract the sample right before the leap.
...
All user-defined landmarks can be removed using `userLandmarks(exBundle) <- NULL`.
...
*Please note*, that the advantage in computation time increases with the number of features; for a small number of features the parallel computing approach may be slower due to its overhead.
...
tsne_result <- plotManifold(bundle, color_by="phenoName", name="state")
latentSpaceSNE(bundle) <- tsne_result
...
plotStateTrajectory(bundle, color_by="phenoName", name="fm143", point_size=1.5, 
                    label_offset=4, component=1)
plotStateTrajectory(bundle, color_by="phenoName", name="origin", point_size=1.5, 
                    label_offset=4, component=1)
plotStateTrajectory(bundle, color_by="featureName", name="OTOA", point_size=5, 
                    label_offset=4, component=1)
plotStateTrajectory(bundle, color_by="featureName", name="ATOH1", point_size=5, 
                    label_offset=4, component=1)
plotStateTrajectory(bundle, color_by="featureName", name="CALB2", point_size=5, 
                    label_offset=4, component=1)
...
plotMap(bundle, color_by="featureName", name="ATOH1", type="raw")
plotMap(bundle, color_by="featureName", name="ATOH1", type="surface.fit")
plotMap(bundle, color_by="featureName", name="ATOH1", type="surface.se")
plotMap(bundle, color_by="featureName", name="ATOH1", type="surface.fit", 
        samples_only=TRUE)
...
plotDynamic(bundle, feature_name=c("TECTA", "OTOA", "ATOH1", "POU4F3", 
...
# Plot manifold
tsne_result <- plotManifold(p1utricle, color_by="phenoName", name="state")
latentSpaceSNE(p1utricle) <- tsne_result
...
# Plot state trajectory graph (similar to Figure 1G)
plotStateTrajectory(p1utricle, color_by="phenoName", name="gate", point_size=1.5, 
                    label_offset=4, component=1)
plotStateTrajectory(p1utricle, color_by="featureName", name="Otoa", point_size=5, 
                    label_offset=4, component=1)
...
plotMap(p1utricle, color_by="featureName", name="Atoh1", type="raw")
plotMap(p1utricle, color_by="featureName", name="Atoh1", type="surface.fit")
plotMap(p1utricle, color_by="featureName", name="Atoh1", type="surface.se")
plotMap(p1utricle, color_by="featureName", name="Atoh1", type="surface.fit", 
        samples_only=TRUE)
